...
if (document.querySelector(".section_maps__maps")) {
  ...
  secionMap.addEventListener("mouseover", function (e) {
    if (e.target.tagName === "path") {
      let leftContainer =
        document.querySelector("#interactive-maps").getBoundingClientRect()
          .left + 35;
      let topContainer =
        document.querySelector("#interactive-maps").getBoundingClientRect()
          .top + 30;
      ...
      let top =
        e.target.getBoundingClientRect().top - topContainer + heightElement / 2;
      let left =
        e.target.getBoundingClientRect().left -
        leftContainer +
        widthElement / 2;
      let addCoordX = e.target.getAttribute("data-add-coordx")
        ? e.target.getAttribute("data-add-coordx")
        : 0;
...
document
  .querySelector(".header-all__bottom__search .search-input")
...
document.addEventListener("click", function (e) {
  if (
    !e.target.classList.contains("header-all__bottom__search") &&
    !e.target.closest(".header-all__bottom__search")
  ) {
...
function deleteViewEsc(target) {
  ...
  if (target.closest("[data-search-mobile]")) {
    target.closest("header").querySelector(".header-picture").style.display =
      null;
  }
}
...
document
  .querySelector(".header_mobile__search svg")
  .addEventListener("click", function (e) {
    if (!this.closest("[data-search]").classList.contains("_active")) {
      this.closest("[data-search]").classList.add("_active");
      this.closest("header").querySelector(".header-picture").style.display =
        "none";
    }
  });

document
  .querySelector(".footer_scroll_top")
  .addEventListener("click", function (e) {
    window.scrollTo(0, 0);
  });
...
function showMenu(target) {
  window.addEventListener("click", function (e) {
    if (
      e.target.classList.contains(target.replace(".", "")) ||
      e.target.closest(target)
    ) {
      // console.log(e.target)
      let targetElement = e.target.closest(target)
        ? e.target.closest(target)
        : e.target;
      let heightElement = e.target.querySelector("ul")
        ? e.target.querySelector("ul").scrollHeight
        : e.target.closest(target).querySelector("ul").scrollHeight;
      let childrenElement = "header-all__bottom__list-children-li";
      if (!targetElement.querySelector("ul").style.height) {
        e.preventDefault();
        if (
          targetElement &&
          (e.target.classList.contains(childrenElement) ||
            e.target.closest("." + childrenElement))
        ) {
          ...
          targetElement.closest(
            ".header-all__bottom__list-children"
          ).style.height =
            e.target.closest(".header-all__bottom__list-children")
              .scrollHeight +
            heightElement +
            "px";
        } else {
        ...
        if (
          (e.target.closest(".dropdown") &&
            e.target.closest(".dropdown").querySelector("ul") &&
            !e.target
              .closest(".dropdown")
              .classList.contains("header-all__bottom__list")) ||
          (e.target.classList.contains("dropdown") &&
            e.target.querySelector("ul"))
        ) {
          e.preventDefault();

          if (
            e.target.classList.contains(
              "header-all__bottom__list-children-li-submenu"
            ) ||
            e.target.closest(".header-all__bottom__list-children-li-submenu")
          ) {
        ...
        targetElement.closest(".header-all__bottom__list-children")
          ? (targetElement.closest(
              ".header-all__bottom__list-children"
            ).style.height =
              e.target.closest(".header-all__bottom__list-children")
...
catalogNavBtn &&
  ...
  catalogNavBtn.addEventListener("click", (e) => {
    ...
    catalogNavBtn.innerText = catalogNav.classList.contains("active")
      ? "Закрыть каталог"
      : "Открыть каталог";
  });
...
const swiperTabsSlider = new Swiper(".tabs__main .swiper", {
  ...
  on: {
    slideChange: function (e) {
      const height =
        e.slides[e.realIndex].querySelector(".tab__wrapper").scrollHeight;
      tabsLinks.forEach((item) => {
        item.classList.remove("tab-link--active");
      });
...
function accordBox(i) {
  ...
  let bodyElement = parentElement.querySelector(
    ".vakansion-page_accordion__body"
  );
  let heightParent = parentElement.querySelector(
    ".vakansion-page_accordion__header"
  ).scrollHeight;
  if (!parentElement.style.height) {
    findAll(".vakansion-page_accordion").forEach((el) => {
      ...
      el.querySelector(".vakansion-page_accordion__body").classList.remove(
        "_show"
      );
    });
